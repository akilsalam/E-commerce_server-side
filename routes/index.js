...
const  {Users,Grocery,Mobiles,Fashions, Electronics, HomeFurniture, Appliances, BeautyToys, TwoWheelers,MensClothes,MensFootwear,MensWatches,Bags,WomensClothes,WomensFootwear,WomensWatches,WomensJewellery,Automotives,Sunglasses,Fragrances,Lights, Carts, WishList} = require('../config/config');
...
try {
  const insertedData1 = cartData1 ? await WishList.insertMany([cartData1]) : [];
  const insertedData2 = cartData2 ? await WishList.insertMany([cartData2]) : [];
  const insertedData3 = cartData3 ? await WishList.insertMany([cartData3]) : []; 
  const insertedData4 = cartData4 ? await WishList.insertMany([cartData4]) : [];
  const insertedData5 = cartData5 ? await WishList.insertMany([cartData5]) : [];
  const insertedData6 = cartData6 ? await WishList.insertMany([cartData6]) : [];
  const insertedData7 = cartData7 ? await WishList.insertMany([cartData7]) : [];
  const insertedData8 = cartData8 ? await WishList.insertMany([cartData8]) : [];
  const insertedData9 = cartData9 ? await WishList.insertMany([cartData9]) : [];
  const insertedData10 = cartData10 ? await WishList.insertMany([cartData10]) : [];
  const insertedData11 = cartData11 ? await WishList.insertMany([cartData11]) : [];
  const insertedData12 = cartData12 ? await WishList.insertMany([cartData12]) : [];
  const insertedData13 = cartData13 ? await WishList.insertMany([cartData13]) : [];
  const insertedData14 = cartData14 ? await WishList.insertMany([cartData14]) : [];
  const insertedData15 = cartData15 ? await WishList.insertMany([cartData15]) : [];
  const insertedData16 = cartData16 ? await WishList.insertMany([cartData16]) : [];
  const insertedData17 = cartData17 ? await WishList.insertMany([cartData17]) : [];
  const insertedData18 = cartData18 ? await WishList.insertMany([cartData18]) : [];
  const insertedData19 = cartData19 ? await WishList.insertMany([cartData19]) : [];
...
router.get('/two-wheelers', async (req, res) => {
  try {
    const products = await TwoWheelers.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/mens-clothes', async (req, res) => {
  try {
    const products = await MensClothes.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/mens-watches', async (req, res) => {
  try {
    const products = await MensWatches.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/mens-footwears', async (req, res) => {
  try {
    const products = await MensFootwear.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/women-clothes', async (req, res) => {
  try {
    const products = await WomensClothes.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/women-watches', async (req, res) => {
  try {
    const products = await WomensWatches.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/women-footwears', async (req, res) => {
  try {
    const products = await WomensFootwear.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/women-jewellery', async (req, res) => {
  try {
    const products = await WomensJewellery.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/automotive', async (req, res) => {
  try {
    const products = await Automotives.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/fragrances', async (req, res) => {
  try {
    const products = await Fragrances.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/lights', async (req, res) => {
  try {
    const products = await Lights.find();
    res.json(products);
  } catch (error) {
    console.error('Error fetching data:', error);
    res.status(500).json({ error: 'Internal Server Error' });
...
router.get('/View/:id', async (req, res) => {
  try {
    ...
    // Assuming Lights and Fragrances have a common structure with _id field
    const lightProduct = await Lights.findById(id);
    const fragranceProduct = await Fragrances.findById(id);
    const sunglassesProduct = await Sunglasses.findById(id);
    const automotive = await Automotives.findById(id)
    const jewellery = await WomensJewellery.findById(id)
    const WomenWatches = await WomensWatches.findById(id)
    const WomenFootwear = await WomensFootwear.findById(id)
    const WomenClothes = await WomensClothes.findById(id)
    const Bag = await Bags.findById(id)
    const Watches = await MensWatches.findById(id)
    const Footwear = await MensFootwear.findById(id)
    const Shirts = await MensClothes.findById(id)
    const Groceries = await Grocery.findById(id)
    const Mobile = await Mobiles.findById(id)
    const Fashion = await Fashions.findById(id)
    const Electronic = await Electronics.findById(id)
    const Furniture = await HomeFurniture.findById(id)
    const Appliance = await Appliances.findById(id)
    const Beauty = await BeautyToys.findById(id)
    const TwoWheeler = await TwoWheelers.findById(id)
    const Cart = await Carts.findById(id)
    const Wish = await WishList.findById(id)
    // Determine which product type to use based on the existence of the product in either collection
    const product = Cart || Wish || lightProduct || fragranceProduct || sunglassesProduct || automotive || jewellery || WomenWatches || WomenFootwear || WomenFootwear || WomenClothes || Bag || Watches || Footwear || Shirts || Groceries || Mobile || Fashion || Electronic || Furniture || Appliance || Beauty || TwoWheeler;
